...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate English syllable embeddings.")
...
def main():
    ...
    model_name = "syllable_embedding_wiki" if args.corpus_source == "wikipedia" else "syllable_embedding"
    em.part_embedding(model_name=model_name, size=150)

    if args.plot:
        model_file_stem = "{0}_en".format(model_name)
